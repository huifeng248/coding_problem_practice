...
def add_lists(head_1, head_2):
  ...
  current2 = head_2
  print("!!!!!", current1.val)
  while current1 is not None or current2 is not None or prev_increment != 0:
    # print("!!!!!", current1.val)
    if current1 is None:
      current1 = Node(0)
    if current2 is None:
    ...
    if (current1.val + current2.val + prev_increment)>= 10:
      value = (current1.val + current2.val + prev_increment) % 10
      prev_increment = 1
      print("bigger than 10", value)
    else: 
      value = current1.val + current2.val + prev_increment
      prev_increment = 0
      print("smaller", value)
...
def add_lists(head_1, head_2, carry=0):
  ...
  result_node = Node(value)
  
  node_1 = None if head_1 is None else head_1.next
  node_2 = None if head_2 is None else head_2.next
  
  result_node.next = add_lists(node_1, node_2, carry)
  return result_node
